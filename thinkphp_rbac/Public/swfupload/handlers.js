...
function preLoad() {
	...
	if (!this.support.loading) {
		alert("你需要Flash Player 9.028以上使用SwfUpload。");
		return false;
	}
}
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		...
		if (numFilesQueued > 0) {
			...
			//this.startResizedUpload(this.getFile(0).ID, 200, 200, SWFUpload.RESIZE_ENCODING.JPEG, 100);
...
function addImage(src) {
	...
	document.getElementById("thumbnails").appendChild(newImg); //将图片标签添加到网页div层中。
	//将图片设置为全透明
